...
| 2026-05-03 | !178 / ef4cdd90be87bcd4f48b293de1cedd8d4f879a10 | Prior concern likely mismatched because weak title-derived identity drifted | no | Follow-up review treated the surviving unconditional-exception concern in `remove_duplicate_oe_numbers` as newly introduced even though it was already present in the prior pass; likely continuity mismatch caused by title/identity drift rather than a true new issue | context + code | tracking | The concern appears semantically unchanged across passes, but current continuity matching still relies heavily on title-derived identity when stronger structured fields such as `symbol`, `issue_kind`, or `region_hint` are missing. |
...
- Typical shape:
  ...
  - in some cases the likely failure is not raw line-number matching, but weak
    title-derived identity drift when semantically identical findings are
    worded slightly differently across passes
- Preferred response:
  ...
  - inspect whether the prior/current finding identities drifted because
    continuity had to fall back to title-derived keys instead of stronger
    structured fields such as `symbol`, `issue_kind`, or `region_hint`
