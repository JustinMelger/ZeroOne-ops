...
def build_overlap_packet(
    *,
    ...
    for current_index, current_finding in enumerate(review_result.findings):
        for prior_index, prior_finding in _candidate_prior_findings(
...
def _candidate_prior_findings(
    *,
    ...
    current_key = _current_finding_key(current_finding)
    ...
    same_file_candidates = [
        (index, prior_finding)
        for index, prior_finding in enumerate(prior_findings)
        if _normalized_file_path(_prior_file_path(prior_finding)) == current_file_path
    ]
...
def _candidate_reasons(
    *,
    ...
    prior_title = _prior_title(prior_finding)
    if prior_title is not None and _titles_look_like_same_finding(
...
def _narrow_candidates_by_field(
    ...
    *,
    ...
    matching_candidates = [
        candidate for candidate in candidates if getattr(candidate[1], field_name) == expected_value
    ]
...
def _prior_file_path(finding: PriorReviewFinding) -> str | None:
    """Extract the normalized file path from one prior finding summary."""
    parsed = _split_prior_summary(finding.summary)
    return parsed[0] if parsed is not None else None


def _prior_title(finding: PriorReviewFinding) -> str | None:
