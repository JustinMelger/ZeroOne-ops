...
class GitLabWorkItemRenderer:
    ...
    def render_title(self, work_item: WorkItemState) -> str:
        """Render one compact GitLab work-item title."""
    ...
    def render_body(self, work_item: WorkItemState) -> str:
        """Render one deterministic GitLab work-item issue body."""
        ...
        lines.extend(["", "## Remediation Merge Request", ""])
        if work_item.linked_change_request is None:
            lines.append("No remediation merge request is linked yet.")
        else:
            lines.extend(
                [
        ...
        if work_item.projected_review is None:
            lines.append("No remediation merge-request review has been projected yet.")
        else:
            lines.extend(
                [
    ...
    def _render_recovery_instructions(work_item: WorkItemState) -> list[str]:
        ...
        if work_item.execution_failure is not None:
            ...
            blocker = "This remediation is blocked because merge-request publication failed."
        else:
